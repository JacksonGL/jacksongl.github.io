...
((function (sandbox){
    var profile = [];
    var trace = [];
    var forbiddenFuns = [Math.abs, Array.prototype.sort];
    ...
    function readPre (iid, name, val, isGlobal) {
        if(isGlobal) {
			...
			console.log('[warning] use of global variable: ' + name);
			if(location){
				console.log('\t location: ' + location[1] + ': ' + location[2]);
			} else {
				console.log('\t[iid]: ' + iid);
			}
    ...
    function invokeFunPre (iid, f, base, args, isConstructor) {
        var location = window.iidToLocationMap[iid];
        for(var i=0;i<forbiddenFuns.length;i++){
            if(forbiddenFuns[i] === f){
                if(location)
                    console.log('line No.: ' + location[1] + ' | colunm No.: ' + location[2] + ' | use of forbidden function: ' + forbiddenFuns[i].name);
                else 
                    console.log('[iid]: ' + iid + '| use of forbidden function: ' + forbiddenFuns[i].name);
            }
        }
    }
