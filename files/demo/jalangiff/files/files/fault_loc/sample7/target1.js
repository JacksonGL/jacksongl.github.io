    ...
    function merge(left, right) {
        ...
        while (left.length || right.length) {
            if (left.length && right.length) {
                if (left[0] < right[0]) {
                    // result.push(left.shift()); // correct statement
                    // result.unshift(left.shift()); // bug pattern-1
                    result.push(right.shift()); // bug pattern-2
                } else {
...
var array = Mergesort.sort(array, 0, array.length-1);
